...
public class Estudiante extends Lector {
    ...
    public Estudiante(int CodE, String Nombre, String Dirección, int Telefono) {
    ...
    public void reservarLibro(ArrayList<Libro> lb, ArrayList<Pedido> pd, ArrayList<Estudiante> est, int CODL, boolean validacion) {
        ...
        for (i = 0; i < lb.size(); i++) {
            ...
            if (lb.get(i).isDisponible() == true) {
                ...
                if (lb.get(i).getCODL() == CODL) {
                    ...
                    pd.add(new Pedido(est.get(i).getCodE(), lb.get(i).getBooktitle(), CODL, r, e));
                }
            }
        }
        if (b == false) {
    ...
    public void reservarRevista(ArrayList<Revista> rv, ArrayList<Pedido> pd, ArrayList<Estudiante> est, int CODR, boolean validacion) {
        ...
        for (i = 0; i < rv.size(); i++) {
            ...
            if (rv.get(i).isDisponible() == true) {
                ...
                if (rv.get(i).getCODR() == CODR) {
                    ...
                    pd.add(new Pedido(est.get(i).getCodE(), rv.get(i).getMagazineTitle(), CODR, r, e));
                }
            }
        }
        if (b == false) {
